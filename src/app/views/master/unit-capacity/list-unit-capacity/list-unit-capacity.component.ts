...
@Component({
  selector: 'app-list-unit-capacity',
  templateUrl: './list-unit-capacity.component.html',
  styleUrls: ['./list-unit-capacity.component.scss']
})
export class ListUnitCapacityComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private _service: UnitCapacityService) {
  }
  ...
  ngOnInit() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        const value: UnitCapacity = this.searchBox.value;
      ...
      columns: [
        {data: 'unitCapacityId', title: 'ID'},
        {data: 'unitCapacityName', title: 'Name'},
        {data: 'unitCapacityDescription', title: 'Description'},
        {
          data: 'unitCapacityId',
          title: 'Action',
          orderable: false,
      ...
      rowCallback: (row: Node, data: UnitCapacity, index: number) => {
        $('button#action-update', row).click(() => {
          this._router.navigate(['master', 'unitCapacity', data.unitCapacityId]);
        });
        $('button#action-remove', row).click(() => {
          this.idDelete = data.unitCapacityId;
  ...
  removed(): void {
    this._service.remove(this.idDelete).subscribe(data => {
      if (data.status === 200) {
        this._toastr.warning('Category of unitCapacity Removed', 'Remove Success',
          {
            timeOut: 4000
          });
        this.refresh(null);
