  ...
  ngOnInit() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        const value: Brand = this.searchBox.value;
      ...
      columns: [
        {data: 'brandId', title: 'ID'},
        {data: 'brandName', title: 'Name'},
        {data: 'brandDescription', title: 'Description'},
        {
          data: 'brandId',
          title: 'Action',
          orderable: false,
      ...
      rowCallback: (row: Node, data: Brand, index: number) => {
        $('button#action-update', row).click(() => {
          this._router.navigate(['master', 'brand', data.brandId]);
        });
        $('button#action-remove', row).click(() => {
          this.idDelete = data.brandId;
